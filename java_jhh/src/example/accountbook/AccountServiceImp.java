...
public class AccountServiceImp implements AccountService {
	...
	@Override
	public void pirntItem(ArrayList<Item> list) {
		if(list.size() == 0) {
	...
	public void runMenu(ArrayList<Item> list, Predicate<Item> p)  throws ParseException {
	...
	@Override
	public void pirntItem(ArrayList<Item> list, int... dates) {
		...
		date = date.substring(1);//년도 앞에 -를 제거하기 위해
	...
	@Override
	public int intputDay(Scanner scan) {
	...
	public void runPrint(ArrayList<Item> list, int subMenu, Scanner scan) {
		...
		case 2:
			year = inputYear(scan);
			pirntItem(list, (s)->s.getDateStr().substring(0, 4).equals(""+year));
			
			break;
		case 3:
			year = inputYear(scan);
			month = inputMonth(scan);
			//pirntItem(list, year, month);
			pirntItem(list, (s)->s.getDateStr().substring(0, 7).equals(""+year+"-"+month));
		...
		case 4:
			...
			month = inputMonth(scan);
			day = intputDay(scan);
			//pirntItem(list, year,month, day);
			pirntItem(list,
					(s)->s.getDateStr().substring(0, 10)
	...
	@Override
	public void pirntItem(ArrayList<Item> list, Predicate<Item> p) {
		for(Item tmp : list) {
			if(p.test(tmp)) {
