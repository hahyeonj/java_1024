...
public interface AccountService {
	...
	void pirntItem(ArrayList<Item> list, int  ...dates);//가계부에 있는 내역 리스트
	
	void pirntItem(ArrayList<Item> list, Predicate<Item> p);
	
	boolean updateItem(ArrayList<Item> list, int index, Item item);//가계부에 있는 내역리스트, 몇번지, 수정할 아이템

	boolean deleteItem(ArrayList<Item> list, int index);
